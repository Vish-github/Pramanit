...
async function getFile(cid) {
  ...
  console.log(`Got a response for death! [${res.status}] ${res.statusText}`);
  if (!res.ok) {
...
const pdfbuilder = async (data, res) => {
  ...
  doc
    ...
    .fontSize(13)
    .text(konkani.deathcertificate, {align: "center"})
    .text(english.deathcertificate, {align: "center"});
  doc.moveDown();
  doc.moveDown();
  ...
  doc
    ...
    .fontSize(11)
    .text(
      `${konkani.name}/${english.name}: ${data.firstName} ${data.lastName} `,
      {
        align: `justify`,
      }
    );
  ...
  doc.text(
    `${konkani.dod}/${english.dod}: ${data.dateOfDeath.split("T")[0]} `,
    {
      align: `justify`,
    }
  );
  ...
  doc.text(
    `${konkani.reasonofdeath}/${english.reasonofdeath}: ${data.reasonOfDeath} `,
    {
      align: `justify`,
    }
  );
  ...
  doc.text(
    `${konkani.addressAtDeath}/${english.addressAtDeath}: ${data.addressAtTimeOfDeath} `,
    {
      align: `justify`,
    }
  );
  ...
  doc.text(
    `${konkani.paddress}/${english.paddress}: ${data.permanentAddress} `,
    {
      align: `justify`,
    }
  );
